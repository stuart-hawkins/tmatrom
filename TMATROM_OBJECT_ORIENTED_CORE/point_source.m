...
%
%   f = u.evaluateFarField(z) returns the values f of the far field of the
%   point source at points z, where the entries in z have unit modulus.
%
...
classdef point_source < radiating_incident
    ...
    methods
        ...
        %-----------------------------------------------
        % evaluate far field method
        %-----------------------------------------------

        function val = evaluateFarField(self,points)
        
            dp = real(self.source)*real(points) + ...
                imag(self.source)*imag(points);
            
            val = (1/sqrt(pi*self.kwave))*(1-1i)*exp(-1i*self.kwave*dp);
