...
classdef wavefunctionexpansion < handle
    ...
    methods
        ...
        %-----------------------------------------
        % evaluate gradient of near field
        %-----------------------------------------
        
        function [dx,dy] = gradient(self,points,mask)
            ...
            % get far field values
            [tx,ty] = self.internal_gradient(points);
    ...
    methods
        ...
        function apply_addition_theorem(self,new_origin,type,new_order)
            ...
            % get Bessel function
            if strcmp(type,'SAME')
                bess = besselj(0:2*max(self.order,new_order),self.kwave*r);
            else
                bess = besselh(0:2*max(self.order,new_order),self.kwave*r);
            end
            B=bess(abs(pp)+1);
